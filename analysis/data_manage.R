...
dt.update.outcomes <- arrow::read_feather( here::here("output","update_outcomes.feather"))
update.names <- names(dt.update.outcomes)[-(names(dt.update.outcomes) == 'patient_id')]

dt <- fread(here::here('output','input.csv'))
dt[,(update.names) := NULL]
dt[dt.update.outcomes, on=.(patient_id), (update.names) := lapply(update.names, function(x) get(paste0('i.',x))) ]
...
dt <- dt.COD[,.(patient_id, death_underlying_cause_ons)][dt[region!=''],,on = 'patient_id']
rm(dt.COD)
####
...
dates <- c('end.fu','gp.end','tstart','tstop','event',
           'discharge.date','date_death_ons','event.VTE','event.readmit',
