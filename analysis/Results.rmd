...
cuminc.adjusted <- 
    matrix(cuminc.cox(n.type.events = n.type.events,dt = 'dt.tv', model = 'post.op.covid.model', newdata = 'new.data.postop', day = 30), byrow = T, ncol = 4)

colnames(cuminc.adjusted) <- paste0('Wave_',1:4)
rownames(cuminc.adjusted) <- paste0(c('Elective','Emergency'),rep(procedures, each = 2))
...
adjusted.cuminc <- cbind(crude.covid.cov,
                         foreach::foreach(predi = 1:length(covariates), .combine = 'c', .inorder = T) %do% {
                           ...
                           cuminc.cox(n.type.events = n.type.events,dt = 'dt.tv', model = 'post.op.covid.model', newdata = 'newdata.pred', day = 30)
                           }
                         )
...
post.op.LOS.model <- 
  lapply(n.type.events, function(i) survival::coxph(survival::Surv(start,end, event.los == i) ~ Abdominal + Cardiac + Obstetrics + Orthopaedic + Thoracic + Vascular + postcovid + age.cat + sex + bmi.cat + imd5 + wave + vaccination.status.factor + region + Current.Cancer + Emergency + Charl12 + recentCOVID + previousCOVID, id = patient_id,
                  data = dt.tv[(postop.los.cohort)], model = T))

knitr::kable(broom::tidy(post.op.LOS.model[[1]], exponentiate= T, conf.int = T),digits = 2)
...
colnames(cuminc.adjusted.los) <- paste0('Wave_',1:4)
rownames(cuminc.adjusted.los) <- paste0(c('No COVID','COVID'),rep(procedures, each = 2))

knitr::kable(cuminc.adjusted.los,col.names = paste0("Wave ",1:4), row.names = T, digits = 2)
