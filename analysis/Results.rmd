...
numbers2words <- function(x){
  ...
  cuminc.km <- function(x,iter)  { 
    n.type.events <- max(dt.tv$event)
  est <-boot.est <- Reduce(merge,lapply(1:n.type.events, function(i) data.table::setDT(summary(survival::survfit(survival::Surv(start,end,event==i) ~ get(x), 
                                                                                 data = dt.tv[(postop.covid.cohort)], 
                                                                                 id = patient_id),
                                                               times = sort(unique(dt.tv[(postop.covid.cohort) & event == i,end]
                                                               )
                                                               )
    )[c('strata','time','n.risk', 'n.event')])[,c('strata','time',paste0('haz',i)) := list(strata,time, n.event/n.risk)][,c(1,2,5)])
    )[order(strata,time),
      .(time,cumsum(exp(cumsum(log(1 - Reduce('+',.SD))))*haz1)),
      by = strata, 
      .SDcols = paste0('haz',1:n.type.events)][order(strata,time),
                                   round(tail(V2,1),digits = 3)*100, 
                                   by = strata] 
  
  for(i in 1:iter) {
    samp.patient_id <- sort(sample(unique(dt.tv[(postop.covid.cohort),patient_id]), replace = T))
    boot.est <- cbind(boot.est,Reduce(merge,lapply(1:n.type.events,
                                                   function(i) data.table::setDT(summary(survival::survfit(survival::Surv(start,end,event==i) ~ get(x), 
                                                                                 data = dt.tv[(postop.covid.cohort)][samp.patient_id,], 
                                                                                 id = patient_id),
                                                               times = sort(unique(dt.tv[event == i,end],
                                                               extend = T
                                                               )
                                                               )
    )[c('strata','time','n.risk', 'n.event')])[
    ,c('strata','time',paste0('haz',i)) := list(strata,time, n.event/n.risk)][,c(1,2,5)])
    )[order(strata,time),.(time,cumsum(exp(cumsum(log(1 - Reduce('+',.SD))))*haz1)),by = strata, .SDcols = paste0('haz',1:n.type.events)][order(strata,time),round(tail(V2,1),digits = 3)*100, by = strata][,2] )
  }
  return(cbind(est[,1],
               rnd(data.table::setDT(summary(survival::survfit(survival::Surv(start,end,event==i) ~ get(x), 
                                                               data = dt.tv[(postop.covid.cohort),],
                                                               id = patient_id),
                                             times = 90)[c('n.risk', 'n.event')])),
               est[,2],
               t(apply(boot.est[,2:(iter + 1)],1,quantile,c(0.025,0.975)))))
  }
...
dt.tv[,event :=0]
dt.tv[tstop == COVIDpositivedate, event := 1]
dt.tv[tstop == readmit.end, event := 2]
dt.tv[died == T, event := 3]

dt.tv[, postop.covid.cohort := start>=0 & tstop <= final.date & tstop <= final.date & end <= 90]
...
crude.covid.cov <- data.table::rbindlist(lapply(1:length(covariates), function(i) cbind(rep(covariates[i],length(levels(dt.tv[(postop.covid.cohort),as.factor(get(covariates[i]))]))),
                                    ...
                                    cuminc.km(covariates[i], iter = 100)[,2:6])))

names(crude.covid.cov) <- c("Characteristic","Level","Number at risk","Number of events","90 day Cumulative Risk","Bootstrapped Confidence","95% interval" )
