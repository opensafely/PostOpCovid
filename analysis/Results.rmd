...
numbers2words <- function(x){
  ...
  for(iter in 1:niter) {
    samp.patient_id <- dt.tv[(sort(sample(unique(dt.tv[(postop.covid.cohort),patient_id]), replace = T))),]
                                                     ...
                                                     if(samp.patient_id[(event==i),.N] > 0) {
                                                        data.table::setDT(summary(survival::survfit(survival::Surv(start,end,event==i) ~ get(x), 
                                                                                  ...
                                                                                  extend = T)[c('strata','time','n.risk', 'n.event')])[
                                                                                    ,c('strata','time',
                                                                                       paste0('haz',i)) := list(strata,time, n.event/n.risk)][,c(1,2,5)]
                                                     }
                                                     else {
    ...
    )[order(strata,time),.(time,cumsum(exp(cumsum(safelog(1 - Reduce('+',.SD))))*haz1)),by = strata, .SDcols = paste0('haz',n.type.events)][order(strata,time),round(tail(V2,1),digits = 3)*100, by = strata][,2] )
  }
  return(cbind(est[,1],
...
crude.covid.cov <- data.table::rbindlist(lapply(1:length(covariates), function(i) cbind(rep(covariates[i],length(levels(dt.tv[(postop.covid.cohort),as.factor(get(covariates[i]))]))),
                                    ...
                                    cuminc.km(covariates[i], niter = 100)[,2:6])))
