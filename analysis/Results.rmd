...
#  boot.est<- foreach::foreach(iter = 1:niter, .combine = 'cbind',.multicombine = T,.export = 'dt.tv',.packages = 'data.table') %dopar% {
  ...
  return(cbind(est[,1],
               rnd(dt.tv[(postop.covid.cohort) & !is.na(get(x)) & tstop == final.date,.N, keyby = get(x)][,2]),
...
dt.tv[,postVTE90.perepisode := max(post.VTE & end <=90,na.rm = T), by = .(patient_id, end.fu)]
dt.tv[!is.finite(postVTE90.perepisode),postVTE90.perepisode := 0]

data.table::setkey(dt.tv,patient_id,tstart,tstop)

demo.tab <- 
  data.table::transpose(cbind(dt.tv[(postop.covid.cohort) & start ==0,
                                 .("Procedures" = rnd(sum(tstart == admit.date, na.rm = T)),
                                 ...
                                 "30 day COVID-19 (%)" = n.perc(date.30.day(x = COVIDpositivedate, ref.dat = admit.date), dig = 3),
...
```{r demo, results='asis', echo = F, warning=F, eval = FALSE}


demo.tab <- 
  data.table::transpose(cbind(dt[date_discharged >= date_admitted | is.na(date_discharged) ,.("Procedures" = rnd(.N),
                                 "Patients" = rnd(length(unique(patient_id))),
                                 "Female" = n.perc(sex=='F',dig = 3),
                                 ...
                                 "IMD quintile 4" = n.perc(as.numeric(imd5) ==  4, dig  = 3),
                                 "IMD quintile 5" = n.perc(as.numeric(imd5) ==  5, dig  = 3),
                                 "Wave 1" = n.perc(admit.wave == 'Wave_1', dig = 3),
                                 "Wave 2" = n.perc(admit.wave == 'Wave_2', dig = 3),
                                 "Wave 3" = n.perc(admit.wave == 'Wave_3', dig = 3),
                                 "Wave 4" = n.perc(admit.wave == 'Wave_4', dig = 3),
                                 ...
                                 "30 day COVID-19 (%)" = n.perc(date.30.day(x = date, ref.dat = date_admitted), dig = 3),
                                 "Recent COVID_19 (%)" = paste0(rnd(sum(recent_case_category != "", na.rm = T))," (", round(100*mean(recent_case_category != "", na.rm = T), digits = 1),"%)"),
                                 "Previous COVID_19 (%)" = paste0(rnd(sum(previous_case_category != "", na.rm = T))," (", round(100*mean(previous_case_category != "", na.rm = T), digits = 1),"%)"),
...
cuminc.adjusted <- apply(exp(apply((log((1 - Reduce('+',lapply(n.type.events, function(i) outer(survival::basehaz(post.op.covid.model[[i]],centered = F)[,1] -
...
knitr::kable(matrix(cuminc.adjusted, byrow = T, ncol = 4))
```
