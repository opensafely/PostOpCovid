...
numbers2words <- function(x){
  ...
  cuminc.km <- function(x,niter)  { 
    ...
    )[,(paste0('haz',n.type.events)) := lapply(.SD, function(x) fifelse(is.na(x),0,x)), .SDcols = paste0('haz',n.type.events)][
      order(strata,time),
...
)[,(paste0('haz',n.type.events)) := lapply(.SD, function(x) fifelse(is.na(x),0,x)),
...
crude.covid.cov <- data.table::rbindlist(lapply(1:length(covariates), function(i) cbind(rep(covariates[i],length(levels(dt.tv[(postop.covid.cohort),as.factor(get(covariates[i]))]))),
                                    ...
                                    cuminc.km(covariates[i], niter = 20)[,2:6])))
