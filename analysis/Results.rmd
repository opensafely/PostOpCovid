...
dt.tv[(tstop - tstart)>90, tstop := tstart + 90]
dt.tv <- dt.tv[end <= 90]
data.table::setkey(dt.tv,patient_id,tstart,tstop)
dt.tv[,bmi := max(bmi, na.rm = T), by = .(patient_id, end.fu) ]
dt.tv[,imd.quin := max(as.numeric(imd5), na.rm = T), by = .(patient_id, end.fu) ]
dt.tv[!is.finite(vaccination.status), vaccination.status := 0]
dt.tv[,vaccination.status := max(vaccination.status, na.rm = T), by = .(patient_id, end.fu) ]
dt.tv[!is.finite(Current.Cancer), Current.Cancer := 0]
dt.tv[,Current.Cancer := max(Current.Cancer, na.rm = T), by = .(patient_id, end.fu) ]
...
dt.tv[,postcovid := max(postcovid, na.rm = T), by = .(patient_id, end.fu) ]
dt.tv[!is.finite(postVTEany), postVTEany := 0]
dt.tv[,postVTEany := max(postVTEany, na.rm = T), by = .(patient_id, end.fu) ]
...
dt.tv[tstop == end.fu & !is.na(op.type),.('Procedures' = rnd(.N),
                         ...
                         "IMD quintile (IQR)" = median.iqr(as.numeric(imd.quin),dig = 0),
                         ...
                         "Emergency" = n.perc(Emergency >= 1,dig = 3),
