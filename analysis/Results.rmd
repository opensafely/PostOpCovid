...
post.op.covid.model <- 
  lapply(1:3, function(i) survival::coxph(survival::Surv(start,end,event==i) ~ op.type + age.cat + sex + bmi.cat + imd5 + wave + vaccination.status.factor + Current.Cancer + Emergency + Charlson + recentCOVID + previousCOVID, id = patient_id,
                  data = dt.tv[(postop.covid.cohort)], model = T))

survival::basehaz(post.op.covid.model[[1]],centered = F)[,1] -c(0,head(survival::basehaz(post.op.covid.model[[1]],centered = F)[,1],-1))*predict(object = post.op.covid.model[[1]],type = 'lp', newdata = data.table::data.table('start' = rep(0,8),
                                                                    'end' = rep(30,8*length(procedures)),
                                                                    'event' = rep(F,8*length(procedures)),
                                                                    'op.type' = rep(procedures,each = 8),
                                                                    'age.cat' = rep('(50,70]',8*length(procedures)),                                                                                                                                                             + age.cat + sex + bmi.cat + imd5 + wave + vaccination.status.factor + Current.Cancer + Emergency + Charlson + recentCOVID + previousCOVID))
