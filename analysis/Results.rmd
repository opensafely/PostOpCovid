...
numbers2words <- function(x){
  ...
  cuminc.km <- function(x,niter)  { 
    ...
    est <- Reduce(merge,lapply(n.type.events, function(i) data.table::setDT(summary(survival::survfit(survival::Surv(start,end,event==i) ~ get(x), 
  ...
  boot.est<- foreach::foreach(iter = 1:niter, .combine = 'cbind',.multicombine = T,.export = 'dt.tv',.packages = 'data.table') %dopar% {
    ...
    data.table::setkey(dt.tv, patient_id, tstart,tstop)
     Reduce(merge,lapply(n.type.events,
                                    function(i) {
