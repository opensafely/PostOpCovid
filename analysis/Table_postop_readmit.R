...
new.data.postop.covid <- data.table::data.table('start.readmit' = rep(0,8*length(procedures)),
                                              ...
                                              #  'Cardiac'=c(rep(F,8*length(procedures) -4*8),rep(T,8),rep(F,8*length(procedures) -2*8)),
                                                'Obstetrics'=c(rep(F,8*length(procedures) -2*8),rep(T,8),rep(F,8*length(procedures) -2*8)),
                                                'Orthopaedic'=c(rep(F,8*length(procedures) -3*8),rep(T,8),rep(F,8*length(procedures) -3*8)),
