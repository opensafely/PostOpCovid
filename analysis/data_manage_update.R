...
dt.update <- Reduce(function(...) {
  merge(..., by = c('patient_id'), all = T)
           ...
           function(x) data.table::fread(here::here('output',paste0('input_',x,'.csv')))[,(paste0(x,'_date_admitted')) := date_admitted][,
...
max.grp.col_(dt = 'dt.update', max.var.name = 'max.date', aggregate.cols = c(proc.time.cols.end,proc.time.cols.start), id.vars = 'patient_id') 
min.grp.col_(dt = 'dt.update', min.var.name = 'min.date', aggregate.cols = c(proc.time.cols.end,proc.time.cols.start), id.vars = 'patient_id') 


# maximum date of follow up in data 
max.date.fu <- max(as.numeric(data.table::as.IDate(dt.update$max.date)), na.rm = T)

dt.update[!is.finite(max.date), max.date := as.numeric(data.table::as.IDate('2022-03-01'))]
dt.update[,tstart := min.date]
...
dt.tv.update <- dt.update[dt.dates.long,,rollends = c(T,T), roll = Inf, on = c('patient_id','tstart'), mult = 'all'][,
