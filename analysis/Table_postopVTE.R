...
post.op.VTE.model <- 
  lapply(n.type.events, function(i) survival::coxph(survival::Surv(start,end,event.VTE==i) ~ Abdominal*wave +  Obstetrics*wave + CardioThoracicVascular*wave + postcovid*wave  + age.cat +
                                            sex +  imd5 + vaccination.status.factor  + Current.Cancer + Emergency + LOS.bin + Charl12 + recentCOVID + previousCOVID, id = patient_id,
...
modelsummary::modelsummary(post.op.VTE.model,estimate  = "{estimate} [{conf.low}, {conf.high}], (p = {p.value})", 
                           ...
                           conf_level = .95, exponentiate = TRUE, output = here::here("output","postopVTEmodel.html"))
modelsummary::modelsummary(post.op.VTE.model,estimate  = "{estimate} [{conf.low}, {conf.high}], (p = {p.value})", 
                           statistic = NULL, gof_omit = "Num.Obs.|n|nevent",
                           conf_level = .95, exponentiate = TRUE, output = here::here("output","postopVTEmodel.txt"))
