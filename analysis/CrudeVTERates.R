...
for (op_type in c('Major.op', 'Intermediate.or.Minor.op')) {
  for (Emerg in c(F, T)) {
    ...
    crude.vte.rates <- cbind(
      rbindlist(
        lapply(covariates, function(x) {
          dt.tv[Emergency == Emerg & get(op_type) == T & (postcovid.VTE.cohort) & !is.na(get(x)) & start >= 0, head(.SD, 1), keyby = .(patient_id, end.fu)][
            , .N, keyby = c(x, 'vte')
    ...
    data.table::fwrite(crude.vte.rates, file = here::here("output", paste0(op_type, "_", Emerg_label, ".csv")))
