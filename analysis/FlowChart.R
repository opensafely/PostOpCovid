...
n.ops.COVID <- rnd(dt.tv[(postcovid.VTE.cohort) & start ==0  & is.finite(admit.date) & any.op == T,lapply(.SD,function(x) sum(x == T)), .SDcols = c(procedures)])

n.pats <- rnd(length(unique(dt.tv[any.op == T,patient_id])))
...
n.VTE.90 <- rnd(dt.tv[any.op == T,max(event.VTE == 1 & end <=90 , na.rm = T), keyby = .(patient_id, end.fu)][,tail(.SD), keyby = .(patient_id, end.fu)][,sum(V1==1)])
n.VTE.90.censored <- rnd(dt.tv[(postcovid.VTE.cohort) & any.op == T,max(event.VTE == 1 & end <=90, na.rm = T), keyby = .(patient_id, end.fu)][,tail(.SD), keyby = .(patient_id, end.fu)][,sum(V1==1)])
n.VTE.30.censored <- rnd(dt.tv[(postcovid.VTE.cohort) & any.op == T,max(event.VTE == 1 & end <=30, na.rm = T), keyby = .(patient_id, end.fu)][,tail(.SD), keyby = .(patient_id, end.fu)][,sum(V1==1)])
n.VTE.7.censored <- rnd(dt.tv[(postcovid.VTE.cohort) & any.op == T,max(event.VTE == 1 & end <=7, na.rm = T), keyby = .(patient_id, end.fu)][,tail(.SD), keyby = .(patient_id, end.fu)][,sum(V1==1)])
