...
n.ops.VTE <- rnd(dt.tv[(postop.covid.cohort) & start ==0  & is.finite(admit.date),lapply(.SD,function(x) sum(x == T)), .SDcols = c(procedures)])
n.ops.COVID <- rnd(dt.tv[(postcovid.VTE.cohort) & start ==0  & is.finite(admit.date),lapply(.SD,function(x) sum(x == T)), .SDcols = c(procedures)])

n.pats <- rnd(length(unique(dt.tv[,patient_id])))
n.pats.study <- rnd(length(unique(dt.tv[(postop.covid.cohort) & start ==0 & is.finite(admit.date) & any.op == T,patient_id])))
n.pats.late <- rnd(length(unique(dt.tv[admit.date > gp.end & any.op == T,patient_id])))

n.covid.90 <- rnd(dt.tv[,max(event == 1 & end <=90, na.rm = T), keyby = .(patient_id, end.fu)][,tail(.SD), keyby = .(patient_id, end.fu)][,sum(V1==1)])
n.covid.90.censored <- rnd(dt.tv[(postop.covid.cohort) ,max(event == 1 & end <=90, na.rm = T), keyby = .(patient_id, end.fu)][,tail(.SD), keyby = .(patient_id, end.fu)][,sum(V1==1)])
n.covid.30.censored <- rnd(dt.tv[(postop.covid.cohort) ,max(event == 1 & end <=30, na.rm = T), keyby = .(patient_id, end.fu)][,tail(.SD), keyby = .(patient_id, end.fu)][,sum(V1==1)])
n.covid.7.censored <- rnd(dt.tv[(postop.covid.cohort) ,max(event == 1 & end <=7, na.rm = T), keyby = .(patient_id, end.fu)][,tail(.SD), keyby = .(patient_id, end.fu)][,sum(V1==1)])

n.VTE.90 <- rnd(dt.tv[,max(event.VTE == 1 & end <=90, na.rm = T), keyby = .(patient_id, end.fu)][,tail(.SD), keyby = .(patient_id, end.fu)][,sum(V1==1)])
n.VTE.90.censored <- rnd(dt.tv[(postcovid.VTE.cohort) ,max(event.VTE == 1 & end <=90, na.rm = T), keyby = .(patient_id, end.fu)][,tail(.SD), keyby = .(patient_id, end.fu)][,sum(V1==1)])
n.VTE.30.censored <- rnd(dt.tv[(postcovid.VTE.cohort) ,max(event.VTE == 1 & end <=30, na.rm = T), keyby = .(patient_id, end.fu)][,tail(.SD), keyby = .(patient_id, end.fu)][,sum(V1==1)])
n.VTE.7.censored <- rnd(dt.tv[(postcovid.VTE.cohort) ,max(event.VTE == 1 & end <=7, na.rm = T), keyby = .(patient_id, end.fu)][,tail(.SD), keyby = .(patient_id, end.fu)][,sum(V1==1)])
