...
n.covid.30.censored <- rnd(dt.tv[,max(event == 1 & start >=0  & end <=30 & (postop.covid.cohort) & any.op == T, na.rm = T), keyby = .(patient_id, end.fu)][,tail(.SD), keyby = .(patient_id, end.fu)][,sum(V1==1)])
