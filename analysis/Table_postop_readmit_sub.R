...
data.table::fwrite(broom::tidy(post.op.readmit.model.sub[[1]], exponentiate= T, conf.int = T), file = here::here("output","postopreadmitmodel_sub.csv"))
...
cuminc.adjusted.readmit.sub <- 
  matrix(cuminc.cox(n.type.events = n.type.events,dt = 'dt.tv[(postcovid.readmit.cohort)]', model = 'post.op.readmit.model.sub', newdata = 'new.data.postop.covid', day = 90), byrow = T, ncol = 4)
