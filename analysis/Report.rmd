...
-   Types of Surgery - Categorised into the following major procedure groups: \* Abdominal procedures (including gastrointestinal, gynaecological, urological procedures) \* Orthopaedic procedures \* Obstetric procedures \* Thoracic procedures
...
For the post operative risk the exposure of interest was the effect of a COVID-19 positive test stratified by the timing of procedure by COVID-19 wave. Note we excluded repeat positive COVID tests identified within 90 days of a previous positive test.
...
covid_coef_sub <- data.table::data.table('cov_level' = names(post.op.covid.model.sub[[1]]$coefficients), 
                                   ...
                                   '95% CI' = paste0('(',round(exp(confint(post.op.covid.model.sub[[1]])[,1]),2),',',round(exp(confint(post.op.covid.model.sub[[1]])[,2]),2),')'))
...
readmission_coef_sub <- data.table::data.table('cov_level' = names(post.op.covid.model.sub[[2]]$coefficients), 
                                   ...
                                   '95% CI' = paste0('(',round(exp(confint(post.op.covid.model.sub[[2]])[,1]),2),',',round(exp(confint(post.op.covid.model.sub[[2]])[,2]),2),')'))
...
mortality_coef_sub <- data.table::data.table('cov_level' = names(post.op.covid.model.sub[[3]]$coefficients), 
                                   ...
                                   '95% CI' = paste0('(',round(exp(confint(post.op.covid.model.sub[[3]])[,1]),2),',',round(exp(confint(post.op.covid.model.sub[[3]])[,2]),2),')'))
...
```{r postopcovid_waves,fig.cap="The adjusted cumulative risk estimates were calculated for each COVID-19 wave stratified by broad operation category in table 2."}
...
```{r postopcovid_waves_sub,fig.cap="The adjusted cumulative risk estimates were calculated for each COVID-19 wave stratified by selected operation category in table 3."}
...
An accelerated failure survival model was fitted to directly estimate the effect of post operative COVID-19 as a time varying covariate on post operative length of stay. Predicted length of stay was calculated stratified by the COVID infection for a 50-70 year old female with a single morbidity with a bmi of 20, for an abdominal cancer operation, without previous COVID-19, with a booster vaccination, from the East Midlands in an area ranked in the 3rd quintile for deprivation.
...
```{r postopVTE_plot,fig.cap="This plot shows the changing effect of post operative COVID infection on VTE riskin the initial, alpha, delta and omicron waves."}
...
```{r postopVTErecentcovid, results='asis', fig.cap="This plot shows the changing effect of recent pre-operative COVID infection on VTE risk in the initial, alpha, delta and omicron waves."}
...
ggplot2::ggplot(weekly.post.op.VTE.risk) + #ggplot2::geom_line(ggplot2::aes(x = Days.post.discharge, y = Risk.risk, group = COVID, colour = COVID)) + 
 ggplot2::geom_col(ggplot2::aes(x = factor(`Risk period`, level = c("Week in hospital","1st post discharge week","2nd week","3rd week","4th week","5th week"),
                                           labels = c("Inpatient","1st post \n discharge week","2nd week","3rd week","4th week","5th week")),
                                                                    y = Risk,
                              group = COVID,
                             colour = COVID,
                            fill = COVID), position = "dodge") +
ggplot2::xlab("Weekly Risk Periods pre and post discharge") + 
...
```{r postopVTE_sub, results='asis', eval = T}
#load(file = here::here("output","postopVTE_sub.RData"))

#knitr::kable(broom::tidy(post.op.VTE.model[[1]], exponentiate= T, conf.int = T),digits = 2)
htmltools::includeHTML(here::here('output','postopVTEmodelsub.html'))
...
```{r postopVTErecentcovid_sub_plot,fig.cap = "This shows the changing effect of recent pre operative COVID infection on VTE risk in the initial, alpha, delta and omicron waves for a 50-70 year old female with a single morbidity with a bmi of 20, for a cancer colectomy operation, without previous COVID-19, with a booster vaccination, from the East Midlands in an area ranked in the 3rd quintile for deprivation."}
load(file = here::here("output","postopVTErecent_sub.RData"))
...
Adjusted Cox proportional hazards survival models were fitted to estimate the effect of post operative COVID-19 as a time varying covariate on mortality, and cumulative incidence functions calculated to adjust for competing risks of readmission.

```{r postopmortab, eval = T, echo=FALSE}

################################
# Post operative COVID risk model----
##################################
load(here::here("output","postopmort_crude.RData"))
load(here::here("output","postopmort_adjusted.RData"))
crude.mort.cov[, cov_level := paste0(Characteristic, Level) ]

mort_coef <- data.table::data.table('cov_level' = names(post.op.mort.model[[1]]$coefficients), 
                                   'Adjusted Hazard Ratio' = round(exp(post.op.mort.model[[1]]$coefficients),2),
                                   '95% CI' = paste0('(',round(exp(confint(post.op.mort.model[[1]])[,1]),2),',',
                                                     round(exp(confint(post.op.mort.model[[1]])[,2]),2),')'))
...
#knitr::kable(broom::tidy(post.op.covid.model[[1]], exponentiate= T, conf.int = T),digits = 2)
procedures <- c('Abdominal','Obstetrics','Orthopaedic','CardioThoracicVascular')
comb_postop_mort <- cbind(merge(x = crude.mort.cov, y = mort_coef, by = 'cov_level', all = T , sort = F),
                           adjusted.mort.cuminc)[-(seq(1,(length(procedures)*2)-1,2)),-1]                           

names(comb_postop_mort) <- c("Characteristic","Level","Number at risk",
                            "Number of post operative deaths within 30 days","30 day Cumulative Risk adjusted for censoring",
                            'Crude Hazard Ratio for 30 day mortality (95% CI)',
                            'Adjusted Hazard Ratio for 30 day mortality','95% CI',
                            "30 day Mortality Cause Specific Cumulative Risk adjusted for all covariates",
                            "Cause specfic 30 day Mortality Cause Specific Cumulative Risk adjusted for all covariates with CI from model")

comb_postop_mort[is.na(`Adjusted Hazard Ratio for 30 day mortality`),`:=`(`Adjusted Hazard Ratio for 30 day mortality` = 1, `95% CI`= "Ref")]

 kableExtra::kable_styling(bootstrap_options = c("striped","hover","condensed","responsive"),
knitr::kable(comb_postop_mort, 
             caption = paste0("Table : Cumulative incidence of 30 day post operative mortality of patients by procedure type with counts rounded to nearest 10. Adjusted predictions set for 50-70 year old female with a single morbidity with a bmi of 20, for an elective major abdominal cancer operation, without previous COVID-19, with a booster vaccination, from the East Midlands in an area ranked in the 3rd quintile for deprivation")
             #at their most frequent population values:\n procedure = ", procedures[which.max(dt.tv[,lapply(.SD,sum,na.rm = T), .SDcols = c(procedures)])], Reduce(lapply(which(covariates == 'age.cat'):which(covariates == 'previousCOVID'), function(i) paste0(covariates[i] ,' = ',max.category(which(covariates == covariates[i])))), f =function(x,y) paste(x,y, collapse = ", ")))
             ) %>% kableExtra::add_header_above(c(" " = 4,"Crude KM analysis" = 3, "Adjusted Cox model analysis" = 3))
)
```
...
```{r, postopmortality_fig, fig.cap = "Predicted mortality was calculated stratified by the COVID infection for a 50-70 year old female  with a single morbidity with a bmi of 20, for an elective abdominal cancer operation, without previous COVID-19, with a booster vaccination, from the East Midlands in an area ranked in the 3rd quintile for deprivation."}
...
ggplot2::ggplot(cuminc.long.labelled[order(Emergency, Operation, postcovid,wave)], #data.table::melt(data.table::data.table(cuminc.adjusted.mortality, keep.rownames = T),
                                 #id.vars = 'rn',
                                 #variable.name = 'Wave',
...
```{r postopmortabsub, eval = T, echo=FALSE}

################################
# Post operative COVID risk model----
##################################
load(here::here("output","postopmort_crude_sub.RData"))
load(here::here("output","postopmort_adjusted_sub.RData"))

crude.mort.cov.sub[, cov_level := paste0(Characteristic, Level) ]

mort_coef.sub <- data.table::data.table('cov_level' = names(post.op.mort.model.sub[[1]]$coefficients), 
                                   'Adjusted Hazard Ratio' = round(exp(post.op.mort.model.sub[[1]]$coefficients),2),
                                   '95% CI' = paste0('(',round(exp(confint(post.op.mort.model.sub[[1]])[,1]),2),',',
                                                     round(exp(confint(post.op.mort.model.sub[[1]])[,2]),2),')'))

procedures.sub <- c('Colectomy','Cholecystectomy',
                'HipReplacement','KneeReplacement')

comb_postop_mort_sub <- cbind(merge(x = crude.mort.cov.sub, y = mort_coef.sub, by = 'cov_level', all = T , sort = F),
                           adjusted.cuminc.mort.sub)[-(seq(1,(length(procedures.sub)*2)-1,2)),-1]

names(comb_postop_mort_sub) <- c("Characteristic","Level","Number at risk",
                            "Number of post operative deaths within 30 days",
...
comb_postop_mort_sub[is.na(`Adjusted Hazard Ratio for 30 day mortality`),`:=`(`Adjusted Hazard Ratio for 30 day mortality` = 1, `95% CI`= "Ref")]

 kableExtra::kable_styling(bootstrap_options = c("striped","hover","condensed","responsive"),
knitr::kable(comb_postop_mort_sub, 
...
```{r, postopmortality_sub_fig, fig.cap = "Predicted mortality was calculated stratified by the COVID infection for a 50-70 year old female  with a single morbidity with a bmi of 20, for an elective colectomy cancer operation, without previous COVID-19, with a booster vaccination, from the East Midlands in an area ranked in the 3rd quintile for deprivation."}
...
ggplot2::ggplot(cuminc.long.labelled.sub[order(Emergency, Operation, postcovid,wave),], #data.table::melt(data.table::data.table(cuminc.adjusted.mortality.sub, keep.rownames = T),
                                                    #     id.vars = 'rn',
                                                     #    variable.name = 'Wave',
                                                      #   value.name = '90 Day Cumulative Mortality Risk (%)')[, `:=`(COVID = grepl('^COVID*',rn),
                                                       #                                                          Operation = gsub('^No COVID|^COVID', '',rn))],
