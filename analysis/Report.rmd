...
# Post operative COVID risk model----
##################################
load(here::here("output","postopcovid_mort_crude.RData"))
load(here::here("output","postopmort_adjusted.RData"))

crude.mort.cov[, cov_level := paste0(Characteristic, Level) ]

mort_coef <- data.table::data.table('cov_level' = names(post.op.mort.model$coefficients), 
                                   'Adjusted Hazard Ratio' = round(exp(post.op.mort.model$coefficients),2),
                                   '95% CI' = paste0('(',round(exp(confint(post.op.mort.model)[,1]),2),',',
                                                     round(exp(confint(post.op.mort.model)[,2]),2),')'))
...
comb_postop_mort <- cbind(merge(x = crude.mort.cov, y = mort_coef, by = 'cov_level', all = T , sort = F),
                           adjusted.mort.cuminc,
                           adjusted.mort.cuminc[,c(4,2)])[-(seq(1,(length(procedures)*2)-1,2)),-1]

names(comb_postop_mort) <- c("Characteristic","Level","Number at risk",
                            "Number of post operative COVID events within 30 days",
                            'Adjusted Hazard Ratio for censoring by 30 day mortality','95% CI',
                            "30 day Mortality Cause Specific Cumulative Risk adjusted for all covariates")

comb_postop_mort[is.na(`Adjusted Hazard Ratio for censoring by 30 day mortality`),`:=`(`Adjusted Hazard Ratio for censoring by 30 day mortality` = 1, `95% CI`= "Ref")]
...
mort_coef.sub <- data.table::data.table('cov_level' = names(post.op.mort.model.sub$coefficients), 
                                   'Adjusted Hazard Ratio' = round(exp(post.op.mort.model.sub$coefficients),2),
                                   '95% CI' = paste0('(',round(exp(confint(post.op.mort.model.sub)[,1]),2),',',
                                                     round(exp(confint(post.op.mort.model.sub)[,2]),2),')'))

procedures.sub <- c('Colectomy','Cholecystectomy',
                'HipReplacement','KneeReplacement')

comb_postop_mort_sub <- cbind(merge(x = crude.mort.cov.sub, y = mort_coef.sub, by = 'cov_level', all = T , sort = F),
                           adjusted.cuminc.mort.sub,
                           adjusted.cuminc.mort.sub[,c(4,2)])[-(seq(1,(length(procedures.sub)*2)-1,2)),-1]
...
comb_postop_mort_sub[is.na(`Adjusted Hazard Ratio for censoring by 30 day mortality`),`:=`(`Adjusted Hazard Ratio for censoring by 30 day mortality` = 1, `95% CI`= "Ref")]
