...
crude.vte.cov <- data.table::rbindlist(
  lapply(1:length(covariates), function(i) {
    ...
    cuminc_values <- cuminc.km.vte(covariates[i], niter = 2, days = 90)[,2:4]
...
names(crude.vte.cov) <- c("Characteristic", "Level", "Number at risk", "Number of post operative VTE events within 90 days", 
                          "90 day Cumulative VTE Risk adjusted for censoring", "Crude Hazard ratio 90 day VTE (95% CI)", "Adjusted Hazard ratio 90 day VTE (95% CI)")

# Save the results to csv and RData
data.table::fwrite(crude.vte.cov, file = here::here("output","postopvte_fullyadjusted90dayanalysis.csv"))
save(crude.vte.cov, file = here::here("output","postopvte_fullyadjusted90dayanalysis.RData"))
