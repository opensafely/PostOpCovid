...
daily.post.op.risk.sub  <-  data.table::data.table(daily.post.op.risk.sub - c(0,daily.post.op.risk.sub[-length(daily.post.op.risk.sub)]),
...
ggplot2::ggplot(daily.post.op.risk.sub) +
...
newdata.pred <- data.table::data.table('start' = rep(0:34, times = 2),
                                       'end' = rep(1:35,times = 2),
                                       'event.VTE' = rep(F,newdata.rows*2),
                                       'week.post.disch' = paste(rep(ceiling(1:35 / 7), times = 2)),
...
######################
ggplot2::ggplot(daily.post.op.VTE.risk.sub) +
