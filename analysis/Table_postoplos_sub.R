...
post.op.LOS.model.sub <-  flexsurv::flexsurvreg(survival::Surv(start,end, event.los == 1) ~ Colectomy + Cholecystectomy + HipReplacement + KneeReplacement + 
...
write.table(tidy.flexsurvreg(v), here::here("output","postopLOSmodel_sub.csv"), sep = ",",quote = F, row.names = F)
